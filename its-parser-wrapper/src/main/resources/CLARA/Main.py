...
class ClaraModelEncoder(JSONEncoder):

    def default(self, obj):

        # YN: extended the naming scheme to match its-baseline
        naming = {
            'Op' : 'Operation',
            'Expr' : 'Expression',
            'Var' : 'Variable',
        ...
        # Add token type for decoding later
        return {
            **obj.__dict__,
            'tokentype': naming[type(obj).__name__]
        }
...
'''
    ...
    def main(self):
        ...
        self.timeout = int(self.opts.pop('timeout', 600))
        ...
        if self.lang == 'py':
            ...
            self.inter = PyInterpreter()
        else:
            self.parser = CParser()
            self.inter = CInterpreter()
        ...
        elif self.cmd == 'jsonmodel':
            print(self.json_model(self.sources[0]))
    ...
    def json_model(self, src):

        with open(src, 'r', encoding="utf-8") as f:
            code = f.read()

        model = self.parser.parse_code(code)
        js = json.dumps(model, cls=ClaraModelEncoder)
        return js
    ...
    def process_source(self, src):
        ...
        model = self.parser.parse_code(code)
        a = json.dumps(model,cls=ClaraModelEncoder)
        model.name = src
        self.extract_exprs(model)
        return a
        # modelEncoder = ClaraModelEncoder()
        # return model
